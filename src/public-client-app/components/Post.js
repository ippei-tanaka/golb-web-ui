    ...
    return (
        <article className="module-post">
            ...
            <div className="m-pst-meta-container">
                <time
                    className="m-pst-time">{getOrdinal(date.getDate())} {getMonthName(date.getMonth() + 1)}, {date.getFullYear()}</time>
                {category ? (
                    <span className="m-pst-category">{category.name}</span>
                ) : null}
            </div>
            <div className="m-pst-content"><Content content={content}/></div>
        </article>
    );
};
